...
## 2026-08-19 — Daily Flow and Clarity milestone

- Changed `Q` into a plant-first cycle. It follows saved inventory order, then reaches cartons, the current customer, and work stations. Empty display positions are no longer keyboard targets. Pointer selection now gives the next `Q` press a stable starting point.
- Added a compact sale-readiness card for the selected plant. Separate rows show availability, display placement, soil moisture, light, condition, helpful care, and the current customer match. The card identifies a hard blocker, a Quick-only condition, or a ready sale. Shops with more than six plants also show the selected plant’s position in the cycle.
- Shortened the old dense selected-plant sentence. Exact readiness details now use readable rows with status marks and full hover text.
- Added **Open all cartons** from Day 6. It keeps the existing flap, packing-paper, lift, and transfer animation at a faster batch pace. The final overview lists each arrival, rescue state, and preferred light before the player starts placement.
- Changed supplier planning to prefer species that are not in current stock. Reliable, Curated, and Mystery Rescue lots maximize unique species before they use request-driven duplicates. Supplier cards show unique and new-to-current-stock counts.
- Made successful Rehabilitation earn 3 Bloom in addition to restored sale value and protected days. The Care Bench job, active-job card, morning result, Help, and README state the reward.
- Updated the title screen to show the saved weekday and day. A non-fresh save now says **Continue Day N** instead of always looking like Day 1.
